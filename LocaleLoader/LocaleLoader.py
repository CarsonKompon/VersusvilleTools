...
print("Versusville Locale Loader v1.0.1")
print("By: Carson Kompon")
print("")
...
def export_obj(map_cell):
    ...
    with open(OBJ_EXPORT_PATH + str(loaded_map) + '\\c' + str(i) + '.obj', 'w') as f:
        for j in range(len(map_cell["texture_list"])):
            ...
            # Write the vertices
            for k in range(face_start * 3, face_start * 3 + face_count * 3):
            ...
            for k in range(face_count):
                f.write(f"f {faces[j][k][0]}/{j} {faces[j][k][1]}/{j} {faces[j][k][2]}/{j}\n")
